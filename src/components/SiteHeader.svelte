...
<header class="site-header flex items-center gap-4 mb-12">
	<a
		href="/"
	...
	{#if title || eyebrow}
		<div class="min-w-0">
			{#if eyebrow}
				<p
					class="text-[10px] font-medium tracking-[0.2em] uppercase text-slate-500 dark:text-slate-400"
				>
					{eyebrow}
				</p>
			{/if}
			{#if title}
				<h1
					class="text-lg font-semibold tracking-tight text-slate-900 dark:text-slate-100 leading-tight truncate"
				>
					{title}
				</h1>
			{/if}
